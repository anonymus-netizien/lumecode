...
class TestBatchCommandIntegration:
    ...
    def test_batch_review_directory(self, runner, sample_project):
        ...
        result = runner.invoke(cli, [
            'batch', 'review',
            '--directory', str(sample_project / "src"),
            '--pattern', '*.py',
            '--provider', 'mock'
        ])
        assert isinstance(result.exit_code, int)
...
class TestCommandOptionCombinations:
    ...
    @pytest.mark.parametrize("streaming,verbose", [
        (True, True),
        (True, False),
        (False, True),
        (False, False),
...
class TestPerformanceSmoke:
    ...
    def test_help_commands_are_fast(self, runner):
        ...
        for cmd in commands:
            ...
            assert result.exit_code == 0
            assert duration < help_timeout, f"{cmd} help took {duration}s (threshold: {help_timeout}s)"
    ...
    def test_mock_provider_is_fast(self, runner):
        ...
        assert duration < mock_timeout, f"Mock provider took {duration}s (threshold: {mock_timeout}s)"
