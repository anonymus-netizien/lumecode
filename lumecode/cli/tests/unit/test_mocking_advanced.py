...
@pytest.mark.mock
@pytest.mark.skip(reason="These tests mock at wrong level - provider is created inside get_provider_with_fallback, not returned by it")
class TestLLMProviderMocking:
    ...
    def test_mock_provider_stream(self, runner):
        ...
        with patch('lumecode.cli.core.llm.get_provider_with_fallback', return_value=mock_provider):
            ...
            assert "Chunk 1" in result.output or "Chunk 2" in result.output or mock_provider.stream_complete.call_count >= 1
    ...
    def test_mock_multiple_provider_calls(self, runner):
        ...
        with patch('lumecode.cli.core.llm.get_provider_with_fallback', return_value=mock_provider):
            ...
            assert mock_provider.complete.call_count == 3, f"Expected 3 calls, got {mock_provider.complete.call_count}"
...
class TestFileSystemMocking:
    ...
    @pytest.mark.skip(reason="Test uses incorrect command syntax - explain code takes FILE_PATH argument, not --file option")
    def test_mock_file_read(self, runner):
        ...
        with patch('lumecode.cli.core.llm.get_provider_with_fallback', return_value=mock_provider):
            with patch('pathlib.Path.read_text', return_value=file_content):
                ...
                assert result.exit_code == 0, f"Command failed: {result.output}"
                assert mock_provider.complete.called, "Provider should have been called with file content"
    ...
    def test_mock_file_not_found(self, runner):
        ...
        with patch('lumecode.cli.core.llm.get_provider_with_fallback', return_value=mock_provider):
            with patch('pathlib.Path.exists', return_value=False):
                ...
                assert not mock_provider.complete.called, "Provider should not be called for missing file"
    ...
    def test_mock_git_no_changes(self, runner):
        ...
        with patch('lumecode.cli.core.llm.get_provider_with_fallback', return_value=mock_provider):
            with patch('subprocess.run', return_value=mock_subprocess):
                ...
                assert result.exit_code == 0, f"Should handle no changes gracefully: {result.output}"
...
class TestCallVerification:
    ...
    def test_verify_provider_called_with_correct_args(self, runner):
        ...
        with patch('lumecode.cli.core.llm.get_provider_with_fallback', return_value=mock_provider) as mock_get:
            result = runner.invoke(cli, [
                'ask', question,
                '--provider', 'mock'
            ])
