...
def list_providers():
    ...
    providers_info = [
        ...
        {"name": "Mock", "key_env": None, "models": "Test provider (offline)", "free": True},
    ]
...
def set_key(
    groq: Optional[str], openrouter: Optional[str], openai: Optional[str], anthropic: Optional[str]
):
    """
    ...
    if not config_updates:
        console.print(
            "[yellow]No API keys provided. Use --groq, --openrouter, --openai, or --anthropic[/yellow]"
        )
        return
...
Streaming: {'Yes' if info.supports_streaming else 'No'}""",
        ...
        response = provider.complete(test_prompt, max_tokens=50, temperature=0.7)

        console.print(Panel(response, title="Test Response", border_style="cyan"))
...
def set_default(provider_name: str, model: Optional[str]):
    ...
    try:
        ...
        if model:
            ...
            console.print(
                f"[green]✓[/green] Default provider set to {provider_name} with model {model}"
            )
        else:
