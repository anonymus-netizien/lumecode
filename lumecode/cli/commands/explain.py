...
@click.option("--lines", "-l", help="Line range to explain (e.g., 10-50, 10:50)")
@click.option("--function", "-f", "function_name", help="Specific function name to explain")
@click.option("--class", "-c", "class_name", help="Specific class name to explain")
@click.option("--examples/--no-examples", default=True, help="Include usage examples")
@click.option("--export", type=click.Path(), help="Export explanation to markdown file")
@click.option("--provider", "-p", default="groq", help="LLM provider to use")
    ...
    try:
        ...
        if function_name:
            ...
            if symbol:
                ...
                if verbose:
                    console.print(
                        f"Extracted {explanation_target} (lines {symbol.start_line}-{symbol.end_line})"
                    )
            else:
                console.print(
                    f"[red]Error:[/red] Function '{function_name}' not found", style="bold"
                )
                return
            ...
            if line_range:
                ...
                console.print(
                    f"[red]Error:[/red] Invalid line range format. Use: 10-50, 10:50, or 10..50",
                    style="bold",
                )
                return
        ...
        prompt = PromptTemplates.explain_code(
            code=code_to_explain, file_path=file_info.path, language=file_info.language
        )
        ...
        console.print(Panel("[yellow]Generating explanation...[/yellow]", border_style="yellow"))
        console.print()

        # Stream the explanation
        streamer = StreamingDisplay(console)
        chunks = llm.stream_complete(prompt=prompt, system_prompt=system_prompt, max_tokens=2000)
...
def concept(question: str, provider: str, verbose: bool):
    ...
    try:
        ...
        # Display question
        console.print(
            Panel(f"[cyan]Concept:[/cyan] {question}", title="❓ Question", border_style="cyan")
        )
        console.print()
...
@explain_group.command()
@click.option("--staged/--unstaged", default=True, help="Explain staged or unstaged changes")
@click.option("--provider", "-p", default="groq", help="LLM provider to use")
@click.option("--verbose", "-v", is_flag=True, help="Show verbose output")
def diff(staged: bool, provider: str, verbose: bool):
    ...
    try:
        ...
        if not diff_text or not diff_text.strip():
            console.print(
                f"[yellow]No {'staged' if staged else 'unstaged'} changes to explain[/yellow]"
            )
            return
        ...
        if verbose:
            ...
            for file_diff in files:
                console.print(
                    f"  • {file_diff.file_path} (+{file_diff.additions}, -{file_diff.deletions})"
                )
            console.print()
        ...
        # Stream the explanation
        console.print(Panel("[yellow]Analyzing changes...[/yellow]", border_style="yellow"))
        console.print()
