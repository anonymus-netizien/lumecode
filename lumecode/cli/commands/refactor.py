...
@click.option("--lines", "-l", help="Specific line range (e.g., 10-50, 10:50)")
@click.option("--export", "-e", type=click.Path(), help="Export suggestions to markdown file")
...
"""
        ...
        chunks = llm.stream_complete(prompt=prompt, system_prompt=system_prompt, max_tokens=3000)
...
@click.option("--export", "-e", type=click.Path(), help="Export analysis to markdown file")
@click.option("--provider", "-p", default="groq", help="LLM provider to use")
@click.option("--verbose", "-v", is_flag=True, help="Show verbose output")
