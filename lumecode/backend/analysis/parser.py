...
class ASTParser:
    ...
    def __init__(self, languages_dir: Optional[str] = None):
        ...
        try:
            ...
            self.logger.info(f"Initialized ASTParser with languages: {', '.join(self.languages.keys())}")
        except ImportError as e:
            logging.error(f"Failed to import Tree-sitter: {e}")
            raise ImportError("Tree-sitter is not installed. Please install it with 'pip install tree_sitter'")
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                try:
                    ...
                    logger.warning(f"Failed to clone {language} repository: {e}. Using fallback method.")
                ...
                try:
                    ...
                    with open(lib_path, "w") as f:
                        f.write(f"# Mock language library for {language}\n# Built with minimal support")
...
def find_nodes_by_type(ast_dict: Dict[str, Any], node_types: Union[str, List[str]]) -> List[Dict[str, Any]]:
    """
...
def find_nodes_by_text(ast_dict: Dict[str, Any], text_pattern: Union[str, re.Pattern], 
...
def find_nodes_by_property(ast_dict: Dict[str, Any], property_name: str, 
                          property_value: Union[str, int, bool, re.Pattern]) -> List[Dict[str, Any]]:
    """
