...
class Message:
    ...
    @classmethod
    def create_error(
        cls, request: "Message", source: str, error: str, details: Optional[Dict[str, Any]] = None
    ) -> "Message":
...
class MessageBus:
    ...
    def unsubscribe(
        self, subscriber_id: str, callback: Optional[Callable[[Message], Awaitable[None]]] = None
    ):
...
class PluginCommunicator:
    ...
    async def request_plugin(
        self, plugin_id: str, action: str, params: Dict[str, Any] = None, timeout: float = 5.0
    ) -> Dict[str, Any]:
        ...
        request = Message.create_request(source=self.agent_id, target=plugin_id, content=content)

        response = await self.message_bus.request(request, timeout)
        return response.content

    async def execute_plugin(
        self, plugin: PluginInterface, method_name: str, params: Dict[str, Any] = None
    ) -> PluginResult:
    ...
    async def broadcast_event(
        ...
        event = Message.create_event(
            source=self.agent_id, event_type=event_type, content=content or {}, priority=priority
        )
