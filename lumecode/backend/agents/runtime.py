...
class AgentRuntime:
    ...
    async def start_agent(
        ...
        if len(self.running_agents) >= self.max_concurrent_agents:
            raise RuntimeError(
                f"Cannot start agent: maximum concurrent agents ({self.max_concurrent_agents}) reached"
            )
    ...
    async def _run_agent_with_timeout(self, execution_id: str) -> None:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                agent.run(context, workspace), timeout=self.max_execution_time
            )
            ...
            execution_context["error"] = (
                f"Agent execution timed out after {self.max_execution_time} seconds"
            )
            execution_context["end_time"] = datetime.now()

            logger.error(
                f"Agent {agent.__class__.__name__} timed out after {self.max_execution_time} seconds"
            )
    ...
    def _check_runtime_status(self) -> None:
        ...
        running_count = sum(
            1
            for ctx in self.running_agents.values()
    ...
    def list_agents(self, status_filter: Optional[AgentStatus] = None) -> List[Dict[str, Any]]:
        ...
        for execution_id in self.running_agents:
            ...
            if (
                ...
                or self.running_agents[execution_id]["status"] == status_filter
            ):
