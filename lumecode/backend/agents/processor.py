...
class ResultProcessor:
    ...
    async def _handle_message(self, message: Message):
        ...
        context = ProcessingContext(
            agent_id=message.source, result_id=message.id, timestamp=message.timestamp
        )
                ...
                try:
                    ...
                    if processed_result:
                        ...
                        self.result_aggregator.add_result(
                            ...
                            priority=ResultPriority.from_str(
                                processed_result.get("priority", "medium")
                            ),
    ...
    async def process_result(
        self, result: Dict[str, Any], context: Optional[ProcessingContext] = None
        ...
        if context is None:
            context = ProcessingContext(
                agent_id="unknown", result_id=str(time.time()), timestamp=time.time()
            )
        ...
        for stage in ProcessingStage:
            ...
            context.processing_history.append({"stage": stage.value, "timestamp": time.time()})

        return processed

    async def _apply_stage(
        self, stage: ProcessingStage, result: Dict[str, Any], context: ProcessingContext
    ) -> Optional[Dict[str, Any]]:
    ...
    async def _apply_rules_sequential(
        self, rules: List[ProcessingRule], result: Dict[str, Any], context: ProcessingContext
    ) -> Optional[Dict[str, Any]]:
    ...
    async def _apply_rules_parallel(
        self, rules: List[ProcessingRule], result: Dict[str, Any], context: ProcessingContext
    ) -> Optional[Dict[str, Any]]:
    ...
    async def process_results(
        ...
        context_factory: Optional[Callable[[Dict[str, Any]], ProcessingContext]] = None,
    ) -> List[Dict[str, Any]]:
        ...
        if self.processing_strategy == ProcessingStrategy.PARALLEL:
            ...
            async def process_one(result):
                ctx = (
                    context_factory(result)
                    if context_factory
                    else ProcessingContext(
                        agent_id="batch", result_id=str(time.time()), timestamp=time.time()
                    )
                )
                ...
                for result in current_batch:
                    ...
                    if processed is not None:
                        ...
                        ctx.processing_history.append(
                            {"stage": stage.value, "timestamp": time.time()}
                        )
