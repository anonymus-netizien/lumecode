...
from backend.analysis.aggregator import (
    ResultAggregator,
    ResultType,
    ResultPriority
)
...
class TestResultAggregator(unittest.TestCase):
    ...
    def test_get_results_by_source(self):
        ...
        self.aggregator.add_result(
            result_type=ResultType.CODE_QUALITY,
            source="agent1",
            data={"message": "Agent 1 issue"}
        )
        
        self.aggregator.add_result(
            result_type=ResultType.SECURITY,
            source="agent2",
            data={"message": "Agent 2 issue 1"}
        )
        
        self.aggregator.add_result(
            result_type=ResultType.PERFORMANCE,
            source="agent2",
            data={"message": "Agent 2 issue 2"}
        )
    ...
    def test_get_results_by_tags(self):
        ...
        security_perf_results = self.aggregator.get_results_by_tags(["security", "performance"], match_all=True)
    ...
    def test_update_result(self):
        ...
        success = self.aggregator.update_result(
            result_id=result_id,
            data={"message": "Updated message", "new_field": "new value"}
        )
